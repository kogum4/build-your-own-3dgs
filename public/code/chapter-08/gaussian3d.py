...
def build_covariance_3d(scale_raw, quaternion_raw):
    ...
    # 対角行列 S を構築
    # NumPyなら np.diag(scale) で一発だが、np.diag はNumPyの関数であり
    # Tensorの_backwardが定義されないため勾配伝播が途切れる。
    # stackで9要素を組み立てればTensor演算として勾配が流れる。
    s0, s1, s2 = scale[0], scale[1], scale[2]
    # requires_grad=Falseの定数なので勾配は流れず、複数のstackで使い回しても問題ない
