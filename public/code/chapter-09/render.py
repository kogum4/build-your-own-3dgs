"""
レンダラー v4: 加重和 + アルファ合成 + Tensor版アルファ合成 + 3Dレンダリング。
第8章: EWA Splatting
"""
...
def render_3d(gaussians, camera, bg_color=(0, 0, 0)):
    ...
    Returns:
        (H, W, 3) のRGB画像 Tensor
    """
    # 通常のimportはファイルの先頭に書きますが、Pythonでは関数の中に
    # import文を書くこともできます。関数が呼ばれたときに初めてモジュールが
    # 読み込まれます。render.pyはprojection.pyに依存するが、
    # projection.pyはrender.pyに依存しない。関数内importにすることで、
    # render.pyの他の関数（v1〜v3）がprojection.pyなしでも動作するようにする。
    # やや特殊なテクニックですが、今は深く考えなくて大丈夫です。
